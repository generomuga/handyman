...
export default function MeTab(props) {
  ...
  const [isContactNoEditable, setIsContactNoEditable] = useState(false);
  ...
  const onPressButton = () => {
    ...
    if (isDisplayNameEditable === true) {
      const [resultIsFullNameEmpty, messageIsFullNameEmpty] =
        validation.isFullNameEmpty(displayName);
      ...
      const [resultIsPhoneNumberEmpty, messageIsPhoneNumberEmpty] =
        validation.isPhoneNumberEmpty(contactNo);
      ...
      const [resultIsPhoneNumberInvalid, messageIsPhoneNumberInvalid] =
        validation.isPhoneNumberInvalid(contactNo);
      if (resultIsPhoneNumberInvalid === true) {
        setErrorMessage(messageIsPhoneNumberInvalid);
        return;
      }

      const [resultIsAddressEmpty, messageIsAddressEmpty] =
        validation.isAddressEmpty(address);
  ...
  return (
    <ScrollView>
      ...
      <View style={style.containerPhoto}>
        <Image
          ...
          source={{
            uri: photoURL
              ? photoURL
              : Image.resolveAssetSource(require("../assets/user.png")).uri,
          }}
        />
      ...
      <View style={{ flexDirection: "row", marginLeft: '4%', marginRight: '4%' }}>
        ...
        <Text style={style.label}>Home address</Text>
      </View>
