...
import { Background, Input, Button, Label } from "./styles";
...
export default function Signup(props) {
  ...
  const [isAgreeOnTermsAndCondition, setIsAgreeOnTermsAndCondition] =
    useState(true);
  ...
  const onSignUp = ({
    ...
    const [resultIsPasswordInvalid, messageIsPasswordInvalid] =
      validation.isPasswordInvalid(password);
    if (resultIsPasswordInvalid === true) {
      setErrorMessage(messageIsPasswordInvalid);
      return;
    }

    const [resultIsPasswordUnequal, messageIsPasswordUnequal] =
      validation.isPasswordUnequal(password, confirmPassword);
    ...
    const [
      resultIsTermsAndConditionAccepted,
      messageIsTermsAndConditionAccepted,
    ] = validation.isTermsAndConditionNotAccepted(isAgreeOnTermsAndCondition);
